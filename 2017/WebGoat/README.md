...
## XSS脆弱性を用いたフィッシング補助
### 学修目標
XSS脆弱性のWEBページがどのようにして｢フィッシング攻撃｣として利用されるか、またログからの検知方法を学修する。
...
被害者が誤まってこのようなフィッシングページで｢検索文字列｣と共に｢スクリプトを含む有害なコード｣がtarget.serverに送信されると、被害者のブラウザにtarget.serverサイトから｢検索結果｣と認証のためにユーザー名とパスワードの入力を促すページが表示される。被害者は、誤まったtarget.serverからの要求と思わずにユーザー名とパスワードを入力してしまうと、攻撃者に被害者の個人情報が漏れてしまう。

### WebGoatの学修
｢フィッシング用ページ｣と｢情報収集サーバ｣を用意する代わりに、脆弱性サーバ(｢Phishing with XSS｣)に｢攻撃コード｣を注入し、認証を促すページを表示させる。認証ためにユーザーIDとパスワードを入力し、ポップアップを表示する。

学修内容としては、｢有害スクリプト｣を含む検索用のフォームを作成する方法と、ブラウザに表示しているページから｢攻撃者の情報収集WEBサイト｣に送信する方法を取得する。

### Steps
  1. ｢Phishing with XSS｣ページを開き、｢Search｣フィルドに作成した｢有害スクリプトを含む認証ページ｣を入力し、「Search｣ボタンをクリックする。

  2. 表示された｢認証ページ｣にユーザーIDとパスワードを入力し、送信する。
  ...
  3. 力した｢ユーザーIDとパスワード｣がポップアップされることを確認する。
...
proxyサーバのログから、"http://webgoat.cyexc-target/WebGoat/start.mvc"でuserとpasswordの情報が漏れていることがわかる。
取得したログはこちら＠
